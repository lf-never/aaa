...
let TaskUtils = {
    ...
    getStatusDriverOrVehicle: async function (vehicleNo, driverId, endDate) {
        ...
        if(vehicleNo) {
            vehicleData = await sequelizeObj.query(`
        ...
        if(driverId) {
            driverData = await sequelizeObj.query(`
        ...
        let stateData = false;
        if(driverData || vehicleData) stateData = true
        return stateData;
    },
    ...
    getFiltrationDriverOrVehicle: async function(indentStartTime, indentEndTime, type, requestId, taskState){
        if(indentStartTime && indentEndTime) {
            const initHotoData = async function (){
                let sql = `
            ...
            let hotoData = await initHotoData()
    ...
    initFromUnitIdList: async function (userId, formHub, formNode){
        let unitIdList = []
    ...
    initNewNotUnitId: async function (hotoRequest){
        let notUnitId = []
    ...
    initStatusName: function (operateType){
        let statusName = null;
    ...
    initUnitIdByHubNode: async function (toHub, toNode){
        let unitId = null
        if(toHub){
            if(toNode) {
                let unit = await Unit.findOne({ where: { unit: toHub, subUnit: toNode }})
                unitId = unit.id
            } else {
                let unit = await Unit.findOne({ where: { unit: toHub }})
                unitId = unit.id
            }
        }
        return unitId
...
module.exports.getVehicleList = async function (req, res) {
    try {
        ...
        let unitIdList = await TaskUtils.initFromUnitIdList(req.cookies.userId, formHub, formNode)
        
        let hotoRequest = await HOTORequest.findOne({ where: { id: requestId } });
        if((!hotoRequest.startTime) || (!hotoRequest.endTime)) return res.json({ data: [], recordsFiltered: 0, recordsTotal: 0 })
        let indentStartTime = moment(hotoRequest.startTime).format('YYYY-MM-DD HH:mm:ss');
        let indentEndTime = moment(hotoRequest.endTime).format('YYYY-MM-DD HH:mm:ss');
        let notUnitId = await TaskUtils.initNewNotUnitId(hotoRequest);
        ...
        const initNotAssignSql = function (){
            if(dataType != 'assign'){
        ...
        const initVehicleOptionSql = function (){
            if(vehicleType){
        ...
        if(dataType != 'view') return res.json({ data: pageResult, recordsFiltered: totalRecord, recordsTotal: totalRecord })

        const initViewData = async function (pageResult, requestId){
            for(let item of pageResult){
                if(item.status == 'Completed' && (moment(item.hotoDateTime).format('YYYY-MM-DD HH:mm') == moment(item.returnDateTime).format('YYYY-MM-DD HH:mm'))){
                    ...
                    if(operationRecordList.length < 1) continue
                    const initCompletedDate = function (){
        ...
        await initViewData(pageResult, requestId)
...
module.exports.getVehicleListByReplace = async function (req, res) {
    try {
        ...
        let hotoRequest = await HOTORequest.findOne({ where: { id: requestId } });
        if((!hotoRequest.startTime) || (!hotoRequest.endTime)) return []
        let indentStartTime = moment(hotoRequest.startTime).format('YYYY-MM-DD HH:mm:ss');
        let indentEndTime = moment(hotoRequest.endTime).format('YYYY-MM-DD HH:mm:ss');
        let notUnitId = await TaskUtils.initNewNotUnitId(hotoRequest)
        ...
        const initNotAssignsSql = function () {
            if(dataType != 'assign'){
                sql += ` and vv.requestId = ?`
                replacements.push(requestId)
                if(dataType == 'return') {
...
module.exports.getDriverList = async function (req, res) {
    try {
        ...
        const initDriverNotAssignSql = function (){
            if(dataType != 'assign') {
        ...
        initDriverNotAssignSql()
        const initDriverAssignSql = function (){
            if(dataType == 'assign') {
                if(excludeData.length > 0) {
                    sql += ` and dd.driverId not in (?)`
                    sql2 += ` and dd.driverId not in (?)`
                    replacements.push(excludeData)
                    replacements2.push(excludeData)
                }

                const initDriverAssignUnitIdSql = function (){
                    if(unitIdList.length > 0){
                        if(notUnitId.length > 0) unitIdList = unitIdList.filter(item => !notUnitId.includes(item));
                        if(unitIdList.length > 0) {
                            sql += ` and dd.id in (?)`
        ...
        const initTypeStateSql = function (){
            if(vehicleType){
        ...
        if(dataType != 'view') return res.json({ data: pageResult, recordsFiltered: totalRecord, recordsTotal: totalRecord })
        const initDriverViewDate = async function (){
            for(let item of pageResult){
                if(item.status == 'Completed' && (moment(item.hotoDateTime).format('YYYY-MM-DD HH:mm') == moment(item.returnDateTime).format('YYYY-MM-DD HH:mm'))){
                    let operationRecordList = await OperationRecord.findAll({ where: { businessId: requestId, businessType: 'hoto', optType: 'assign', afterData: { [Op.not]: null } } })
                    if(operationRecordList.length < 1) continue
                    const initDriverCompletedDate = function (){
                        for(let item2 of operationRecordList){
                            let newDataList = null;
                            let dataList = item2.afterData.split(',')
                            if(dataList.length > 0) {
                    ...
                    initDriverCompletedDate()
                }
            }
        }
        await initDriverViewDate()
        return res.json({ data: pageResult, recordsFiltered: totalRecord, recordsTotal: totalRecord })
    } catch (error) {
        log.error(error)
        return res.json(utils.response(0, error));
...
module.exports.getDriverListByReplace = async function (req, res) {
    try {
        ...
        const initDriverNotAssignSql2 = function (){
            if(dataType != 'assign') {
                sql += ` and dd.requestId = ?`
                replacements.push(requestId)
                if(dataType == 'return') {
        ...
        initDriverNotAssignSql2()
        const initDriverAssignSql2 = function (){
            if(dataType == 'assign') {
                if(excludeData.length > 0) {
                    sql += ` and dd.driverId not in (?)`
                    replacements.push(excludeData)
                }
                const initDriverUnitSql = function (){
                    if(unitIdList.length > 0){
                        if(notUnitId.length > 0) unitIdList = unitIdList.filter(item => !notUnitId.includes(item));
                        if(unitIdList.length > 0) {
                            sql += ` and dd.id in (?)`
...
module.exports.operateRequestById = async function (req, res) {
    try {
        ...
        await sequelizeObj.transaction(async transaction => {
            ...
            const initHotoDriverOrVehicleList = async function (){
                ...
                let hotoDriverOrVehicleList = await sequelizeObj.query(hotoDriverOrVehicleListSql, 
                    { type: QueryTypes.SELECT, replacements: replacementsByhotoDriverOrVehicleList }
                )
                return hotoDriverOrVehicleList
            }
            let hotoDriverOrVehicleList = await initHotoDriverOrVehicleList();
            
            
            const initErrorList = async function (){
                let errorList = []
                for(let item of hotoDriverOrVehicleList){
                    let driver = null;
                    if(item.driverId) driver = await Driver.findOne({ where: { driverId: item.driverId } })
                    let errorName = item.vehicleNo || driver.driverName;
                    const checkHotoVehicleOrDriver = async function (){
                        if(operateType.toLowerCase() != 'rejected' && operateType.toLowerCase() != 'cancelled'){
                            ...
                            if(hotoByDriverOrVehicle) {
                                errorList.push( `${ errorName } In this time frame has been hoto.`)
                            }
                        }
                    }
                    await checkHotoVehicleOrDriver();

                    let state = await TaskUtils.verifyDriverOrVehicleByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    if(state.length > 0){
                        errorList.push( `${ errorName } The operation cannot be performed because some tasks are not completed.`)
                    }
                    let state2 = await TaskUtils.getUrgentStatusByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    if(state2.length > 0){
                        errorList.push( `${ errorName } Some Urgent tasks were not completed, so the operation could not be completed.`)
                    }
                    ...
                    if(loanOutByDriverOrVehicle) {
                        errorList.push( `${ errorName } It's been loaned out.`);
                    }
                }
                return errorList
            }
            let errorList = await initErrorList()
            if(errorList.length > 0) {
                throw errorList
            }
            
            let statusName = TaskUtils.initStatusName(operateType)
            
            const updateHotoStatusInitRemarksName = async function (){
                ...
                if(hotoIdList) {
                    if(hotoIdList.length > 0) {
                        ...
                        const initHotoRemarksName = async function (){
                            let rename;
                        ...
                        remarksName = await initHotoRemarksName()
                    }
                } else {
                    if(operateType.toLowerCase() != 'endorsed') {
            ...
            let remarksName = await updateHotoStatusInitRemarksName()

            const updateHOTORequest = async function (){
            ...
            await updateHOTORequest();

            const initHotoOperationRecord = async function () {
...
const operateRequestById2 = async function (operateType, requestId, hotoIdList, userId) {
    try {
        ...
        await sequelizeObj.transaction(async transaction => {
            const initHotoDriverOrVehicleListSql = async function (){
                let hotoDriverOrVehicleListSql = `
                SELECT driverId, vehicleNo, startDateTime, endDateTime FROM hoto WHERE requestId = ?
                `
                ...
                hotoDriverOrVehicleListSql += `  group by id`
                let hotoDriverOrVehicleList = await sequelizeObj.query(hotoDriverOrVehicleListSql, 
                    { type: QueryTypes.SELECT, replacements: hotoDriverOrVehicleListByReplacements }
                )
                return hotoDriverOrVehicleList
            }
            let hotoDriverOrVehicleList = await initHotoDriverOrVehicleListSql()
            ...
            let dataList = [];
            let remarksName = null;
            const updateHotoInitDataRemark = async function (){
                if(hotoIdList) {
                    if(hotoIdList.length > 0) {
                        if(operateType.toLowerCase() != 'endorsed') {
                            await HOTO.update({ status: operateType, updatedAt: moment().format('YYYY-MM-DD HH:mm:ss') }, { where: { id: hotoIdList } })
                        }
                        const initNameDataList = async function (){
                            for(let item of hotoIdList){
                                let hoto = await HOTO.findOne({ where: { id: item } })
                                if(hoto.driverId){
                                    remarksName = 'hoto driver'
            ...
            await updateHotoInitDataRemark()

            const updateHotoRequestAndOperationRecord = async function (){
                if(operateType.toLowerCase() == 'approved'){
                    let HotoApproved = await HOTO.findAll({ where: { requestId: requestId, status: 'Approved' } })
                    let hotoRequestList = await HOTORequest.findOne({ where: { id: requestId } })
                    if(hotoRequestList.resourceQty == HotoApproved.length) {
            ...
            await updateHotoRequestAndOperationRecord()
        }).catch(error => {
           throw error
        })
    } catch (error) {
...
module.exports.createHoto = async function (req, res) {
    try {
        ...
        if(!hotoList[0].requestId) return res.json(utils.response(0, ` The current request does not exist. `));
        const checkRequestHoto = async function (){
            ...
            const smallHotoResource = function (){
            ...
            const moreHotoResource = function (){
                if(hotoStillNeedNo <= 0){
                    newHotoList = []
                    hotoDriverOrVehicle = hotoList.slice(hotoStillNeedNo, hotoList.length)
                    if(hotoDriverOrVehicle) {
            ...
            if(errorName.length > 0) errorMessageList.push(`${ errorName.join(", ") } Assigned Qty cannot be greater than resource Qty.`)
        }
        await checkRequestHoto()
        if(newHotoList.length <= 0) return res.json(utils.response(0, errorMessageList));

        const checkDriverOrVehicle = async function (){
            for(let item of newHotoList){
                let unitId = await TaskUtils.initUnitIdByHubNode(item.toHub, item.toNode);
                ...
                let state = await TaskUtils.verifyDriverOrVehicleByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                let state2 = await TaskUtils.getUrgentStatusByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                const intitErrorMessageListByLoanTask = async function (){
                    if(statusDriverOrVehicle) {
                        let __errorName4 = item.vehicleNo ? `vehicle(vehicleNo: ${ item.vehicleNo })` : `driver(driverName: ${ driver.driverName })`
                        errorMessageList.push(`Operation failed. The ${ __errorName4 }'s status has changed.`);
                    }
                    let __errName = item.vehicleNo || driver.driverName
                    if(loanOutByDriverOrVehicle) {
                        errorMessageList.push(`${ __errName } It's been loaned out.`);
                    }
                    if(hotoByDriverOrVehicle) {
                        errorMessageList.push(`${ __errName } In this time frame has been hoto.`);
                    }

                    if(state.length > 0){
                        errorMessageList.push(`${ __errName } The operation cannot be performed because some tasks are not completed.`);
                    } 
                    if(state2.length > 0){
                        errorMessageList.push( `${ __errName } Some Urgent tasks were not completed, so the operation could not be completed.`)
                    }
                }
                await intitErrorMessageListByLoanTask();
...
module.exports.createHotoRecord = async function (req, res) {
    try {
        ...
        for(let item of hotoIdList){
            ...
            const initErrorMessageListNewHotoIdList = async function () {
                let __errorMessageName = hotoItem.vehicleNo || driver.driverName
                ...
                if(state.length > 0){
                    errorMessageList.push(`${ __errorMessageName } The operation cannot be performed because some tasks are not completed.`);
                }
                ...
                if(state3.length > 0){
                    errorMessageList.push( `${ __errorMessageName } Some Urgent tasks were not completed, so the operation could not be completed.`)
                }
                ...
                if(state2){
                    errorMessageList.push(`${ __errorMessageName } It's been loaned out.`);
                } 
                if(state.length <= 0 && !state2 && state3.length <= 0){
                    newHotoIdList.push(item)
                }
            }
            await initErrorMessageListNewHotoIdList();
        }
...
module.exports.createHotoRequest = async function (req, res) {
    try {
        ...
        hotoRequest.node = hotoRequest.node && hotoRequest.node != '' ? hotoRequest.node : null
        await sequelizeObj.transaction(async transaction => {
...
module.exports.getHotoRequest = async function (req, res) {
    try {
        ...
        let unitIdList = await TaskUtils.initFromUnitIdList(req.cookies.userId, selectHub, selectNode)
        ...
        const initTableTypeSql = function (){
            if(!status){
                if(tableType) {
        ...
        const initDataList = async function (){
            for(let item of hotoRecord){
                ...
                if(item.status.toLowerCase() == 'approved' && (item.resourceQty == hotoRecordList.length || hotoList.length <= 0)) {
                ...
                const initDataStatus = async function (){
    ...
    try {
        ...
        await sequelizeObj.transaction(async transaction => {
            ...
            for(let item of hotoDriverOrVehicleList){
                ...
                if(item.driverId) driver = await Driver.findOne({ where: { driverId: item.driverId } })
                const initErrorList = async function (){
                    let __errorName = item.vehicleNo || driver.driverName
                    let state = await TaskUtils.verifyDriverOrVehicleByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    if(state.length > 0){
                        errorList.push( `${ __errorName } The operation cannot be performed because some tasks are not completed.`)
                    }
                    let state3 = await TaskUtils.getUrgentStatusByDate(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    if(state3.length > 0){
                        errorList.push( `${ __errorName } Some Urgent tasks were not completed, so the operation could not be completed.`)
                    }
                    let loanOutByDriverOrVehicle = await TaskUtils.getLoanOutDriverOrVehicle(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    if(loanOutByDriverOrVehicle) {
                        errorList.push( `${ __errorName } It's been loaned out.`);
                    }
...
module.exports.replaceHotoByResource = async function (req, res) {
    try {
        ...
        let ultimatelyHotoList = [];
        let errorMessageList = [];
        // body data(hoto) check
        const checkDriverOrVehicleByHoto = async function (){
            for(let item of hotoList){
                ...
                item.creator = userId;
                let driver
                const initDriverHubNode = async function (){
                    if(item.driverId){
                ...
                await initDriverHubNode()
    
                const initVehicleHubNode = async function (){
                    if(item.vehicleNo) {
                ...
                await initVehicleHubNode();
    
                const initErrorListAndUltimatelyHotoList = async function (){
                    //2023-07-14 loan out driver/vehicle not hoto
                    let loanOutByDriverOrVehicle = await TaskUtils.getLoanOutDriverOrVehicle(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    let hotoByDriverOrVehicle = await TaskUtils.getHotoDriverOrVehicle(item.vehicleNo, item.driverId, item.startDateTime, item.endDateTime)
                    let statusDriverOrVehicle = await TaskUtils.getStatusDriverOrVehicle(item.vehicleNo, item.driverId, item.endDateTime)
                    if(statusDriverOrVehicle) {
                        let __errorName = item.vehicleNo ? `vehicle(vehicleNo: ${ item.vehicleNo })` : `driver(driverName: ${ driver.driverName })`
                        errorMessageList.push(`Operation failed. The ${ __errorName }'s status has changed.`);
                    }
                    let __errname = item.vehicleNo || driver.driverName;
                    if(loanOutByDriverOrVehicle) {
                        errorMessageList.push(`${ __errname } It's been loaned out.`);
                ...
                await initErrorListAndUltimatelyHotoList()
            }  
        }
        await checkDriverOrVehicleByHoto()
  
        if(ultimatelyHotoList.length <= 0) return res.json(utils.response(0, errorMessageList));

        const replaceHotoErrorByOldTask = async function (item, resultItem, driver, errorMessageList) {
            ...
            moment(item.endDateTime).format('YYYY-MM-DD HH:mm:ss') >= moment(resultItem.endDateTime).format('YYYY-MM-DD HH:mm:ss'))) {
                ...
                let __errorName3 = driver ? `driver(${ driver.driverName })` : `vehicle(${ item.vehicleNo })`
                errorMessageList.push(`The ${ __errorName3 } from/to time must include the period (${ moment(resultItem.startDateTime).format('YYYY-MM-DD HH:mm:ss') } ~ ${ moment(resultItem.endDateTime).format('YYYY-MM-DD HH:mm:ss') })`)
                throw errorMessageList
            }
        }
        ...
        for(let item of ultimatelyHotoList){
            ...
            let driver = null;
            let hotoObj = [];
            const initHotoByRequestId = async function (){
                if(item.driverId){
                    driver = await Driver.findOne({where: { driverId: item.driverId }})
                    if(driver.nric) {
            ...
            await initHotoByRequestId();

            let oldHoto = await HOTO.findOne({ where: { id: item.id } })
            log.warn(`old hoto ${ JSON.stringify(oldHoto) }`)
            const getDataState = function (){
                let stateData = false
                if(oldHoto.vehicleNo) {
                    if(oldHoto.vehicleNo == item.vehicleNo) stateData = true
                }
                if(oldHoto.driverId) {
                    if(oldHoto.driverId == item.driverId) stateData = true
                }
                return stateData
            }
            let stateData = getDataState()
            if(stateData) continue
            const initErrorMessageListByOldHoto = async function (){
                //2023-07-14 loan out driver/vehicle not hoto
                ...
                let __oldErrorName = oldHoto.vehicleNo || oldHotoByDriver.driverName;
                if(loanOutByDriverOrVehicle) {
                    errorMessageList.push(`${ __oldErrorName } It's been loaned out.`);
                }
                if(state.length > 0){
                    errorMessageList.push(`${ __oldErrorName } The task has started and cannot be replaced.`);
                } 
                if(state3.length > 0){
                    errorMessageList.push( `${ __oldErrorName } Some urgent tasks have been started, causing operations to fail.`)
                }
                if(loanOutByDriverOrVehicle || state.length > 0 || state3.length > 0){
                    throw errorMessageList
                }
            }
            await initErrorMessageListByOldHoto();
            ...
            && moment(item.endDateTime).format('YYYY-MM-DD HH:mm:ss') <= moment(oldHoto.endDateTime).format('YYYY-MM-DD HH:mm:ss'))) {
                const updateHoto = async function (){
                    await sequelizeObj.transaction(async transaction => {
                        if(oldHoto.vehicleNo){
                ...
                if(urgentDutyList.length < 1 || result.length < 1) continue
                const updateTaskOrSysByMV = async function (){
                    for(let resultItem of result){
                        if(!(moment(item.startDateTime).format('YYYY-MM-DD HH:mm:ss') <= moment(resultItem.startDateTime).format('YYYY-MM-DD HH:mm:ss') 
                        && moment(item.endDateTime).format('YYYY-MM-DD HH:mm:ss') >= moment(resultItem.endDateTime).format('YYYY-MM-DD HH:mm:ss'))) continue
                        const updateTask = async function (){
                            ...
                            const updateTaskByDriver = async function (){
                                if(item.driverId) {
                        ...
                        await updateTask()
                        await replaceHotoErrorByOldTask(item, resultItem, driver, errorMessageList)
                    }
                }
                await updateTaskOrSysByMV();
                ...
                const updateUrgentIndent = async function (){
                    for(let resultItem of urgentDutyList){
                        ...
                        && moment(item.endDateTime).format('YYYY-MM-DD HH:mm:ss') >= moment(resultItem.endDateTime).format('YYYY-MM-DD HH:mm:ss'))){
                            ...
                            const updateUrgent = async function (){
                                let remarksName = null;
                                const updateUrgentByVehicle = async function (){
                                    if(item.vehicleNo) {
                                        let vehicle = await Vehicle.findOne({ where: { vehicleNo: item.vehicleNo } })
                                ...
                                await updateUrgentByVehicle();

                                const updateUrgentByDriver = async function (){
                                    if(item.driverId) {
                ...
                await updateUrgentIndent()
            } 
            const initOldError = async function (){
                let __prdfixErrorName = driver ? 'driver('+driver.driverName+')' : 'vehicle('+item.vehicleNo+')';   
                if(hotoObj.length > 0){
                    errorMessageList.push(`Hoto already has ${ __prdfixErrorName }.`)
                } else if(!(moment(item.startDateTime).format('YYYY-MM-DD HH:mm:ss') >= moment(oldHoto.startDateTime).format('YYYY-MM-DD HH:mm:ss') 
                && moment(item.endDateTime).format('YYYY-MM-DD HH:mm:ss') <= moment(oldHoto.endDateTime).format('YYYY-MM-DD HH:mm:ss'))){
                    errorMessageList.push(`The ${ __prdfixErrorName } was out of time.`)
                }
            }
            await initOldError()
        } 
        if(errorMessageList.length > 0) return res.json(utils.response(0, errorMessageList));
        return res.json(utils.response(1, true));
    } catch (error) {
        log.error(error)
        return res.json(utils.response(0, error));
